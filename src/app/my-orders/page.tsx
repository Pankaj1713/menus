...
export default function OrdersPage() {
    ...
    function OrderItem({ orderId, date, amount, items, status }: OrderItemProps) {
        const getStatusBadge = () => {
            ...
            return (
                <span className={`px-2 py-1 text-sm rounded-lg ${statusStyles[status]}`}>
                    #{status}
                </span>
            )
        }
        return (
            <div onClick={() => setOpen(true)} className="bg-white cursor-pointer card-shadow rounded-xl p-4 space-y-2">
                <div className="flex border-b border-gray-900 pb-2 border-dashed   justify-between items-center">
                    <div className="space-y-2">
                        <p className="lg:text-lg text-md font-semibold">Order ID: {orderId}</p>
                   </div>
                    {getStatusBadge()}
                </div>
                <div className="">
                <p className="lg:text-sm text-sm text-gray-500">Order Placed : {date}</p>
        ...
        return (
            <div className="wrapper mt-28 lg:!mt-34 min-h-[80vh] lg:mb-20 md:mb-10 mb-5 lg:w-[60%] mx-auto space-y-6">
                <AnimateEachElement className="space-y-4">
                    {[1, 2, 3].map((_, index) => (
                        <div key={index} className="bg-white card-shadow rounded-xl p-4 space-y-2 animate-pulse">
        ...
        <div className="wrapper lg:mt-40 mt-28 lg:!mt-34 min-h-[80vh] lg:mb-20 md:mb-10 mb-5 lg:w-[60%] mx-auto space-y-6">
            ...
            <ViewOrderDetail
                ...
                items={[
                    ...
                    { name: "Pepsi", quantity: 1, price: 15.00 },
                ]}
                itemTotal={96.05}
                taxes={23.16}
                grandTotal={119.21}
